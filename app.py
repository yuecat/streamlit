...
def get_df_ec():
    ...
    Item_list = soup.find('ul', attrs={'id': 'itemList'})
    ...
    for item in items:
        ...
        datum_ec['title'] = item.find(
            'p', attrs={'class': 'items-grid_itemTitleText_b58666da'}).text
        price = item.find(
            'p', attrs={'class': 'items-grid_price_b58666da'}).text
        datum_ec['price'] = int(price.replace('¥ ', '').replace(',', ''))
        datum_ec['link'] = item.find('a')['href']
        is_stock = item.find(
            'p', attrs={'class': 'items-grid_soldOut_b58666da'}) == None
